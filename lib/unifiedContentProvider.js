...
function filterLocalData(data, options = {}) {
  ...
  if (options.order) {
    filtered.sort((a, b) => {
      ...
      if (aVal == null) return options.order.ascending ? -1 : 1;
      if (bVal == null) return options.order.ascending ? 1 : -1;
...
export async function createUnifiedContentProvider() {
  ...
  console.log(`   ✓ Mode: ${hasSupabase && hasLocalContent ? 'MERGED' : hasSupabase ? 'Supabase only' : hasLocalContent ? 'Local only' : 'NO DATA'}\n`);

  return {
    /**
     * Get all courses from both sources
     * @param {Object} options - Query options
    ...
    /**
     * Get all modules from both sources
     * @param {Object} options - Query options
    ...
    /**
     * Get all lessons from both sources
     * @param {Object} options - Query options
    ...
    /**
     * Get all profiles/users from both sources
     * @param {Object} options - Query options
    ...
    getSourceStatus() {
      return {
        ...
        mode: hasSupabase && hasLocalContent ? 'merged' : 
              hasSupabase ? 'supabase-only' : 
              hasLocalContent ? 'local-only' : 'no-data',
      };
    },
  };
}
