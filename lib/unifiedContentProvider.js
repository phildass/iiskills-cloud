...
function createSupabaseClient() {
  ...
  // Use positive validation: check if values look like real Supabase credentials.
  // This avoids embedding literal placeholder strings that would appear in bundles.
  const hasValidUrl =
    !!supabaseUrl && /^https:\/\/[a-z0-9]+(?:-[a-z0-9]+)*\.supabase\.co\/?$/.test(supabaseUrl);
  const hasValidKey =
    !!supabaseKey && supabaseKey.startsWith("eyJ") && supabaseKey.length >= 100;
...
/**
 * Get app ID from item using multiple possible fields
