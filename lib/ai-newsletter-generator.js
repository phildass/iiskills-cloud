...
function parseAIResponse(aiText, course, editionNumber) {
  ...
  sections.title = titleMatch
    ? titleMatch[1].trim()
    : `Skilling #${editionNumber}: ${course.title} 🚀`;
  sections.subject_line = subjectMatch
    ? subjectMatch[1].trim()
    : `🎯 New Skill Alert: ${course.title}`;
  sections.intro_text = introMatch
    ? introMatch[1].trim()
    : "Hey there! 👋 We just dropped something awesome...";
  sections.course_summary = summaryMatch ? summaryMatch[1].trim() : course.short_description;
  sections.highlights_section = highlightsMatch
    ? highlightsMatch[1].trim()
    : formatHighlights(course.highlights);
...
function generateFallbackContent(course, editionNumber) {
  ...
  return {
    ...
    fun_fact:
      "💡 Did you know? People who invest in upskilling are 3x more likely to advance in their careers. That could be you!",
  ...
  <div class="container">
    ...
    <div class="content">
      ...
      <div class="highlights">
        <h3>✨ What's Inside:</h3>
        <ul>
          ${content.highlights_section
            .split("\n")
            .map((h) => (h.trim() ? `<li>${h}</li>` : ""))
            .join("")}
        </ul>
      </div>
