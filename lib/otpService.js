 ...
 * @param {string} [params.email] - User's email address (required if phone is not provided)
 * @param {string} [params.phone] - User's phone number in E.164 format (required if email is not provided)
 ...
 * @param {string} [params.leadName] - Full name of the lead/recipient (stored for admin-generated OTPs)
...
export async function generateAndDispatchOTP({
  ...
  // Validate required fields — at least one of email or phone must be provided
  if (!email && !phone) {
    throw new Error("Either email or phone is required");
  }
  if (!appId || !appName) {
    throw new Error("appId and appName are required");
  }
  ...
  if (normalizedEmail) {
    ...
    if (invalidateError) {
      console.error("[otpService] Failed to invalidate previous OTPs by email:", invalidateError);
    }
    ...
    if (invalidateError) {
      console.error("[otpService] Failed to invalidate previous OTPs by phone:", invalidateError);
    }
  }
...
export async function sendThankYouEmail({ email, appId, appName, paymentTransactionId }) {
  ...
  try {
    ...
    console.log(
      `[otpService] Thank-you email sent to ${email} for app ${appId} (txn: ${paymentTransactionId})`
    );
    return true;
  } catch (error) {
