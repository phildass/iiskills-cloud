 ...
 * 
 * NOTE: For access control (free/paid, bundles, user access checks),
 * use @iiskills/access-control package instead. This file maintains
 * routing and UI metadata only.
 * 
 * When adding a new app:
 * 1. Add entry to APPS object below
 * 2. Add entry to @iiskills/access-control/appConfig.js for access control
 * 3. Ensure app uses same Supabase credentials
 * 4. Configure app's login page to use UniversalLogin
 * 5. Add callback URL to Supabase dashboard
...
/**
 * Get the post-payment return URL for a paid app.
 *
 * Returns `https://<appname>.iiskills.cloud/authorised` for known paid apps,
 * or falls back to the main app's payment success page for unknown courses.
 * Always returns a production HTTPS URL (never a localhost URL).
 *
 * @param {string} appId - App identifier (e.g., 'learn-management')
 * @returns {string} Full HTTPS return URL
 */
export function getPaymentReturnToUrl(appId) {
  const app = APPS[appId];
  if (app && app.primaryDomain && !app.isFree) {
    return `https://${app.primaryDomain}/authorised`;
  }
 ...
 * @param {string} domain - Full domain (e.g., 'learn-ai.iiskills.cloud' or 'iiskills.cloud')
 * @returns {Object|null} App configuration object or null if not found
 */
