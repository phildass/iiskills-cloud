  ...
  return (
    ...
    >
      <div className="bg-yellow-500 text-black font-bold px-4 py-2 rounded-lg shadow-2xl border-2 border-yellow-600 animate-pulse">
        ...
        {setupMode && <div className="text-xs mt-1 opacity-90">Admin Setup Mode</div>}
        {authSuspended && <div className="text-xs mt-1 opacity-90">Auth Suspended</div>}
        {testMode && !setupMode && !authSuspended && (
          <div className="text-xs mt-1 opacity-90">Test Mode Active</div>
        )}
      </div>
    </div>
  );
