...
export default function CareerMapper() {
  ...
  const generatePDF = async () => {
    ...
    // Footer
    doc.setFontSize(8);
    doc.text('Generated by iiskills.cloud Career Mapper', 20, 280);
        ...
        <main className="flex-1 p-4 lg:p-8">
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="font-semibold">🎯 Complete 3 Missing Links</p>
                <p className="text-sm opacity-90">Unlock premium career insights!</p>
              </div>
              <div className="text-2xl font-bold">
                {Math.min(missingLinks.length, 3)}/3
              </div>
            </div>
          </motion.div>
