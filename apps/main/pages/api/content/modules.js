...
if (!supabaseUrl || !supabaseKey) {
  console.error(
    "[content/modules] Missing Supabase env vars (NEXT_PUBLIC_SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY)"
  );
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!courseId && course_slug) {
      let slugQuery = supabase.from("courses").select("id").eq("slug", course_slug);
