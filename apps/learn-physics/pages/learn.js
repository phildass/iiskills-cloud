...
export default function Learn() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (DISABLE_AUTH) {
        console.log('⚠️ TESTING MODE: Auth bypassed - granting full access to learn-physics');
        setUser({
          id: 'test-user',
  ...
  const getLevelProgress = (level) => {
    ...
    const completedCount = level.modules.reduce((sum, module) => {
      return (
        sum + module.lessons.filter((lesson) => isLessonCompleted(module.id, lesson.id)).length
      );
    }, 0);
      ...
      <main className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 py-12 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white p-8 rounded-lg shadow-lg mb-8">
            ...
            <p className="text-xl text-charcoal mb-4">Continue your physics mastery journey</p>
          ...
          <div className="space-y-6">
            ...
            {physicsCurriculum.levels.map((level, levelIndex) => {
              ...
              return (
                <div
                  key={level.id}
                  className={`bg-white rounded-lg shadow-lg border-l-4 ${colorClass} overflow-hidden`}
                >
                  {/* Level Header */}
                  <div
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4">
                          <span className="text-sm text-gray-600">
                            ...
                            {level.modules.reduce((sum, m) => sum + m.lessons.length, 0)} lessons
                          </span>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                                  ...
                                  return (
                                    ...
                                    >
                                      <div className="flex items-center gap-2">
                                        <span
                                          className={`text-lg ${completed ? "text-green-600" : "text-gray-400"}`}
                                        >
                                          {completed ? "✓" : "○"}
                                        </span>
                                        <div className="flex-1">
