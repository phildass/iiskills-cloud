...
async function migrateCourses(courses: any[]): Promise<Map<string, string>> {
  ...
  log(
    `✅ Courses: ${stats.courses.created} created, ${stats.courses.updated} updated, ${stats.courses.errors} errors`
  );
  return idMapping;
}
...
async function migrateModules(
  ...
  log(
    `✅ Modules: ${stats.modules.created} created, ${stats.modules.updated} updated, ${stats.modules.errors} errors`
  );
  return idMapping;
}
...
async function migrateLessons(
  ...
  log(
    `✅ Lessons: ${stats.lessons.created} created, ${stats.lessons.updated} updated, ${stats.lessons.errors} errors`
  );
  return idMapping;
}
    ...
    try {
      ...
      for (const state of country.children || []) {
        ...
        for (const district of state.children || []) {
          const { error: districtError } = await supabase.from("geography").upsert(
            {
              name: district.name,
              type: district.type,
...
async function migrateGovernmentJobs() {
  ...
  for (const [jobId, jobData] of Object.entries(deadlinesData) as any) {
    try {
      ...
      const eligibility =
        eligibilityTemplates[jobType as keyof typeof eligibilityTemplates] ||
        eligibilityTemplates.clerk;

      const { error } = await supabase.from("government_jobs").upsert(
        {
          job_id: jobId,
  ...
  log(
    `✅ Government Jobs: ${stats.governmentJobs.created} created, ${stats.governmentJobs.errors} errors`
  );
}
...
async function main() {
  ...
  try {
    ...
    console.log(
      `  Questions:      ${stats.questions.created} created, ${stats.questions.errors} errors`
    );
    console.log(
      `  Geography:      ${stats.geography.created} created, ${stats.geography.errors} errors`
    );
    console.log(
      `  Govt Jobs:      ${stats.governmentJobs.created} created, ${stats.governmentJobs.errors} errors`
    );
    console.log("");
