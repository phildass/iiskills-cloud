...
function validateApp(appName) {
  ...
  for (const { file, data, type } of courses) {
    ...
    if (data.sourceApp !== appName) {
      warnings++;
      log(
        `⚠ ${path.basename(file)}: sourceApp is "${data.sourceApp}" but should be "${appName}"`,
        "yellow"
      );
    }
  }
  ...
  const orphanedModules = modules.filter(
    (m) => !m.data.course_id || !courseIds.has(m.data.course_id)
  );
  const orphanedLessons = lessons.filter(
    (l) => !l.data.module_id || !moduleIds.has(l.data.module_id)
  );

  if (orphanedModules.length > 0) {
    warnings++;
    log(
      `\n⚠ Found ${orphanedModules.length} orphaned modules (not linked to any course)`,
      "yellow"
    );
  }
  ...
  if (orphanedLessons.length > 0) {
    warnings++;
    log(
      `\n⚠ Found ${orphanedLessons.length} orphaned lessons (not linked to any module)`,
      "yellow"
    );
  }
