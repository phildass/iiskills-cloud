...
This is a standalone Next.js application for the **Learn AI** module of iiskills.cloud. It is designed to be deployed as a separate service on the subdomain `learn-ai.iiskills.cloud`.
...
1. **Vercel** (Recommended)
   - Deploy directly from the `learn-ai` directory
   - Set domain to `learn-ai.iiskills.cloud`
   - Add environment variables in Vercel dashboard

2. **VPS with Nginx + PM2**
   - Build the app and run with PM2 on port 3007
   - Configure Nginx reverse proxy for `learn-ai.iiskills.cloud`
   - See main [DEPLOYMENT.md](../DEPLOYMENT.md) for details

3. **Docker**
   - Build Docker image with production build
   - Run container on port 3007
...
| `NEXT_PUBLIC_SITE_URL`          | App URL                | `https://learn-ai.iiskills.cloud` |
