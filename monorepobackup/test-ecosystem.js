...
async function main() {
  ...
  if (hasErrors) {
    ...
    info(
      '     (Windows PowerShell: Get-ChildItem -Directory -Filter "learn-*" | ForEach-Object { Push-Location $_.FullName; npm install; Pop-Location })'
    );
    info("  2. Build all apps:");
    info('     npm run build && for dir in learn-*/; do (cd "$dir" && npm run build); done');
    info(
      '     (Windows PowerShell: Get-ChildItem -Directory -Filter "learn-*" | ForEach-Object { Push-Location $_.FullName; npm run build; Pop-Location })'
    );
