...
async function testContentDiscovery() {
  ...
  console.log(
    `✓ Discovered ${allContent.courses.length} courses from ${allContent._metadata.totalFilesFound} files`
  );
...
async function testAppIsolation(appsFound) {
  ...
  for (const appId of appsFound) {
    ...
    const totalItems =
      appContent.courses.length +
      appContent.modules.length +
      appContent.lessons.length +
      appContent.profiles.length +
...
async function testUnifiedProvider(appsFound) {
  ...
  for (const appId of appsFound.slice(0, 3)) {
    // Test first 3 apps
...
async function testStats() {
  ...
  console.log("\nBy Source:");
  console.log(
    `  - Supabase: ${stats.sources.supabase.courses} courses, ${stats.sources.supabase.modules} modules, ${stats.sources.supabase.lessons} lessons`
  );
  console.log(
    `  - Local: ${stats.sources.local.courses} courses, ${stats.sources.local.modules} modules, ${stats.sources.local.lessons} lessons`
  );
  console.log(
    `  - Discovered: ${stats.sources.discovered.courses} courses, ${stats.sources.discovered.modules} modules, ${stats.sources.discovered.lessons} lessons`
  );
  ...
  if (stats.perApp && Object.keys(stats.perApp).length > 0) {
    ...
    Object.entries(stats.perApp).forEach(([appId, counts]) => {
      console.log(
        `  ${appId}: ${counts.courses} courses, ${counts.modules} modules, ${counts.lessons} lessons`
      );
    });
  }
  console.log();
