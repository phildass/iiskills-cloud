...
function checkHeroManagerUsage(appName) {
  ...
  if (
    content.includes('from "./HeroManager"') ||
    content.includes('from "../../../components/shared/HeroManager"')
  ) {
...
function generateReport(results) {
  ...
  console.log("=== Summary ===");
  console.log(
    `Apps with SiteHeader: ${results.summary.appsWithSiteHeader}/${results.summary.totalApps}`
  );
  console.log(
    `Apps with HeroManager: ${results.summary.appsWithHeroManager}/${results.summary.totalApps}`
  );
  ...
  Object.entries(results.apps).forEach(([appName, data]) => {
    console.log(`\n${appName}:`);
    console.log(
      `  SiteHeader: ${data.siteHeader.hasSiteHeader ? "✓" : "✗"} ${data.siteHeader.location ? `(in ${data.siteHeader.location})` : ""}`
    );
    console.log(
      `  HeroManager: ${data.heroManager.usesHeroManager ? "✓" : "✗"} ${data.heroManager.method ? `(${data.heroManager.method})` : ""}`
    );
...
const hasIssues =
  results.summary.appsWithSiteHeader < results.summary.totalApps ||
