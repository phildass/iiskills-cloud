...
export default function AdminModules() {
  ...
  const filteredModules =
    selectedApp === "all" ? modules : modules.filter((module) => module.sourceApp === selectedApp);
        ...
        {!loading && modules.length > 0 && (
          <div className="bg-amber-50 border-l-4 border-amber-400 p-4 mb-6">
            ...
            <ul className="text-sm text-amber-700 list-disc ml-4 space-y-1">
              <li>
                Modules without a level (basic/intermediate/advanced):{" "}
                <strong>{modules.filter((m) => !m.level).length}</strong> — assign a level via
                Supabase or the local content generator.
              </li>
              <li>
              ...
              <li>
                Each lesson should have a <strong>Lesson Quiz (5 questions, pass ≥ 4/5)</strong>.
                Missing quizzes will show as empty in the lesson page.
              </li>
            </ul>
          </div>
        )}
