...
process.env.ADMIN_JWT_SECRET =
  process.env.ADMIN_JWT_SECRET || "test-secret-key-for-testing-only-32-chars";
...
(async () => {
  ...
  try {
    ...
    const token = jwt.sign({ admin: true, timestamp: Date.now() }, secret, { expiresIn: "24h" });
  ...
  try {
    ...
    const shortLivedToken = jwt.sign({ admin: true, timestamp: Date.now() }, secret, {
      expiresIn: "1s",
    });
