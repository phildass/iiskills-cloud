...
try {
  ...
  const allContent = [...aptManifest.items, ...jobsManifest.items, ...cricketManifest.items];
  ...
  const biharJobs = allContent.filter(
    (item) => item.type === "job" && item.location?.state === "Bihar"
  );
  ...
  const jobsWithDeadlines = allContent.filter((item) => item.type === "job" && item.deadline);
  ...
  const iasPrep = allContent.filter((item) =>
    item.tags?.some(
      (tag) => tag.toLowerCase().includes("ias") || tag.toLowerCase().includes("civil-services")
    )
  );
  ...
  console.log(
    `- Content types: test (${tests.length}), job (${jobs.length}), lesson (${lessons.length}), article (${articles.length})`
  );
