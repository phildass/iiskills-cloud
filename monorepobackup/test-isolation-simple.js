...
async function runTests() {
  try {
    ...
    console.log(
      `✓ Found ${allContent._metadata.totalFilesFound} files in ${allContent._metadata.totalAppsScanned} apps`
    );
    console.log();
    ...
    for (const appId of appsArray.slice(0, 3)) {
      ...
      const total =
        appContent.courses.length + appContent.modules.length + appContent.lessons.length;
