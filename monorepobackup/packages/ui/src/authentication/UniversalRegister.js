...
export default function UniversalRegister({
  ...
  const validateForm = () => {
    ...
    if (!simplified) {
      ...
      if (formData.country === "india") {
        ...
        if (!formData.specifyCountry.trim())
          newErrors.specifyCountry = "Please specify your country";
      }
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const emailConfirmRedirect = isClient ? `${window.location.origin}${targetPath}` : undefined;
      ...
      <div className="max-w-2xl w-full space-y-8 bg-white p-10 rounded-xl shadow-2xl">
          ...
          {redirectPath && (
            <div className="mt-4 bg-blue-50 border-l-4 border-blue-400 p-4 rounded">
              <p className="text-sm text-blue-800">
                <strong>📝 Registration Required:</strong> Please create an account to access this
                content. You only need to register once to access all iiskills.cloud apps.
              </p>
            </div>
          )}
        </div>
        ...
        {showGoogleAuth && (
          <div>
            ...
            {/* Strong recommendation for iiskills.cloud email */}
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  <p className="text-sm font-semibold text-yellow-800 mb-2">
                    💡{" "}
                    <strong>
                      We strongly recommend registering directly with your iiskills.cloud account
                    </strong>
                  </p>
                  <p className="text-xs text-yellow-700">
                    Most generic email addresses do not include your name or required personal
                    details. Direct registration ensures your learning profile is secure,
                    personalized, and can be fully supported in the future.
                  </p>
                </div>
              </div>
            </div>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
            ...
            <>
              ...
              {formData.country === "others" && (
                <div>
                  ...
                  />
                  {errors.specifyCountry && (
                    <p className="mt-1 text-sm text-red-600">{errors.specifyCountry}</p>
                  )}
                </div>
              )}
            </>
          ...
          <div className="mt-4 p-4 bg-blue-50 rounded-lg">
            <p className="text-xs text-blue-800">
              <strong>✓ Universal Access:</strong> This account works across all iiskills.cloud apps
              including main site, Learn-Apt, Learn-JEE, Learn-NEET, and all other learning modules.
            </p>
          </div>
        </form>
      </div>
