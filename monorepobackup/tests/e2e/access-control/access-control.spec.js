...
test.describe("Universal Access Control", () => {
  ...
  test.describe("Payment Flow with Bundle", () => {
    test.skip("Purchasing learn-ai should unlock learn-developer (requires payment)", async ({
      page,
    }) => {
      // This test requires:
      // 1. Test user account
  ...
  test.describe("Admin Dashboard", () => {
    test.skip("Admin should see access control dashboard (requires admin account)", async ({
      page,
    }) => {
      // Login as admin
      // await page.goto(`${BASE_URL}/login`);
...
test.describe("Access Control Package Functions", () => {
  test("Package exports correct functions", async () => {
    ...
    const { isFreeApp, isBundleApp, requiresPayment, getBundleInfo, getAppsToUnlock } =
      accessControl;
