    ...
    PAID_APPS.forEach((appId) => {
      test(`${appId}: Unauthenticated user sees landing page`, async ({ page }) => {
        ...
        const hasPaymentInfo = await page
          .locator("text=/buy now/i, text=/purchase/i, text=/upgrade/i, text=/₹/i")
          .count();
        expect(hasPaymentInfo).toBeGreaterThan(0);
      });

      test(`${appId}: Authenticated user without payment sees paywall`, async ({
        page,
        context,
      }) => {
        ...
        const isGated =
          ...
          (await page
            .locator('text=/upgrade required/i, text=/access denied/i, button:has-text("Buy Now")')
            .count()) > 0;
