...
(async () => {
  ...
  if (totalChanges > bigPRThreshold) {
    warn(
      `⚠️ This PR is quite large (${totalChanges} lines changed). Consider breaking it into smaller PRs for easier review.`
    );
  }
  ...
  if (!pr.body || pr.body.length < 50) {
    fail(
      "❌ PR description is too short. Please use the PR template and provide adequate details."
    );
  }
  ...
  if (!issueRegex.test(pr.body)) {
    warn(
      '⚠️ No issue linkage found. Consider linking this PR to relevant issues using "Closes #123" or "Related to #456".'
    );
  }
  ...
  if (consoleLogCount > 0) {
    warn(
      `⚠️ Found ${consoleLogCount} new console.log statement(s). Consider using proper logging or removing debug statements.`
    );
  }
  ...
  if (todoCount > 3) {
    warn(
      `⚠️ Found ${todoCount} new TODO/FIXME comments. Consider creating issues for these tasks.`
    );
  }
  ...
  if (localImportViolations.length > 0) {
    fail(
      "❌ **Prohibited Pattern**: Local component imports detected. Apps should use `@iiskills/ui` package instead."
    );
  ...
  if (deprecatedRefs.length > 0) {
    warn(
      "⚠️ References to deprecated apps detected. Ensure they are properly marked as deprecated/archived."
    );
  }
  ...
  const testFiles = allFiles.filter(
    (file) => file.includes(".test.") || file.includes(".spec.") || file.includes("tests/")
  );

  const hasSourceChanges = jsFiles.some(
    (file) => !file.includes(".test.") && !file.includes(".spec.") && !file.includes("tests/")
  );
  ...
  const envFiles = allFiles.filter((file) => file.includes(".env") && !file.includes(".example"));

  if (envFiles.length > 0) {
    fail(
      "❌ **Security Risk**: Direct .env files should not be committed. Use .env.example files instead."
    );
  }
  ...
  if (packageJsonFiles.length > 0) {
    ...
    if (!allFiles.includes("yarn.lock")) {
      warn(
        "⚠️ package.json was modified but yarn.lock was not updated. Did you run `yarn install`?"
      );
    }
  }
  ...
  if (uiFiles.length > 0) {
    warn(
      "📸 UI/Visual changes detected. Please attach screenshots (desktop, tablet, mobile) to the PR description."
    );

    // Check if PR body mentions screenshots
    const hasScreenshots =
      pr.body.toLowerCase().includes("screenshot") ||
      pr.body.includes("![") ||
      pr.body.includes("<img");

    if (!hasScreenshots) {
      fail(
        "❌ UI changes detected but no screenshots found in PR description. Please run `./capture-qa-screenshots.sh` and attach results."
      );
    } else {
  ...
  for (const file of jsFiles) {
    ...
    for (const line of addedLines) {
      ...
      if (line.includes("dangerouslySetInnerHTML")) {
        securityIssues.push({
          file,
          type: "Usage of dangerouslySetInnerHTML",
          line: line.substring(0, 50),
        });
      }
    }
  ...
  if (hasSignificantChanges && docFiles.length === 0) {
    warn(
      "⚠️ Significant changes detected but no documentation updates. Consider updating relevant docs."
    );
  }
  ...
  if (deletedFiles.length > 0) {
    message(
      `🗑️ ${deletedFiles.length} file(s) deleted. Make sure these deletions are intentional and documented.`
    );

    if (deletedFiles.length > 10) {
      warn(
        "⚠️ Large number of files deleted. Please ensure this is intentional and all references are updated."
      );
    }
  }
  ...
  report +=
    "*For questions about this report, see [PR Requirements Guide](docs/PR_REQUIREMENTS_GUIDE.md)*\n";
