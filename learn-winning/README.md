...
This is a standalone Next.js application for the **Learn Winning** module of iiskills.cloud. It is designed to be deployed as a separate service on the subdomain `learn-winning.iiskills.cloud`.
...
1. **Vercel** (Recommended)
   - Deploy directly from the `learn-winning` directory
   - Set domain to `learn-winning.iiskills.cloud`
   - Add environment variables in Vercel dashboard

2. **VPS with Nginx + PM2**
   - Build the app and run with PM2 on port 3003
   - Configure Nginx reverse proxy for `learn-winning.iiskills.cloud`
   - See main [DEPLOYMENT.md](../DEPLOYMENT.md) for details

3. **Docker**
   - Build Docker image with production build
   - Run container on port 3003
...
| `NEXT_PUBLIC_SITE_URL`          | App URL                | `https://learn-winning.iiskills.cloud` |
