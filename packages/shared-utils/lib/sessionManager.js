...
export function recordAppVisit() {
  ...
  try {
    ...
    const updatedHistory = [currentApp.id, ...history.filter((id) => id !== currentApp.id)].slice(
      0,
      10
    ); // Keep last 10 apps
...
export function getBestAuthRedirect(explicitRedirect = null) {
  ...
  if (explicitRedirect) {
    ...
    const isOurDomain =
      explicitRedirect.includes("iiskills.cloud") || explicitRedirect.includes("localhost");
  ...
  if (!mainApp) {
    ...
    return {
      appId: "main",
      path: "/",
      fullUrl: typeof window !== "undefined" ? `${window.location.origin}/` : null,
    };
  }
