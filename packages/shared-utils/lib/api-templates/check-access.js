...
function getSupabaseClient() {
  ...
  if (!supabaseServiceKey) {
    throw new Error(
      "Missing SUPABASE_SERVICE_ROLE_KEY - access check requires service role privileges"
    );
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      bundledApps: Object.values(appsStatus.bundleAccess || {}).flatMap(
        (bundle) => bundle.unlockedApps || []
      ),
