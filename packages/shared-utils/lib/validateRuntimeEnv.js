...
function validateRuntimeEnvironment() {
  ...
  // Detect if this is the admin section of main app
  const isAdminApp =
    (process.env.APP_NAME && process.env.APP_NAME.includes("admin")) ||
    // Check if we're in the main app's admin section
    (process.cwd && process.cwd().includes("/apps/main/pages/admin"));
  ...
  // Check for required Supabase variables (unless using local content)
