 ...
 */
export function filterByContentType(modules: Module[], types: ContentType[]): Module[] {
 ...
 */
export function filterByTags(modules: Module[], tags: string[], matchAll = false): Module[] {
 ...
 */
export function applyFilters(modules: Module[], filters: ModuleFilters): Module[] {
  let result = modules;
  ...
  if (filters.difficulty && filters.difficulty.length > 0) {
    result = result.filter(
      (module) =>
        module.metadata.difficulty && filters.difficulty?.includes(module.metadata.difficulty)
    );
  }
 ...
 */
export function calculateProgress(module: Module, completedSections?: string[]): number {
