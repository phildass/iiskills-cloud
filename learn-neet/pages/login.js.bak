...
import { supabase, getCurrentUser, signInWithEmail } from '../lib/supabaseClient'
...
export default function Login() {
  ...
  const checkUser = async () => {
    ...
    if (user) {
      const redirect = router.query.redirect || '/learn'
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Sign in to your Learn NEET account" />
      </Head>
      ...
      <main className="min-h-screen bg-gradient-to-br from-blue-900 via-purple-800 to-blue-700 flex items-center justify-center py-12 px-4">
        <div className="max-w-md w-full">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-blue-200">Sign in to continue your preparation</p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-8">
            ...
            {error && (
              <div className={`mb-4 p-4 rounded-lg ${error.includes('Check your email') ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                {error}
              </div>
            )}
