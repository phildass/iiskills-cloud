...
function isNextJsApp(pkg) {
  return pkg.dependencies && pkg.dependencies.next && pkg.scripts && pkg.scripts.start;
}
...
module.exports = {
  ...
  for (const assignment of portAssignments) {
    ...
    const needsPortOverride =
      assignment.source === "auto-assigned" || assignment.source === "reassigned";
    ...
    const dirPath =
      app.relativeDir === "." ? "__dirname" : `path.join(__dirname, '${app.relativeDir}')`;
...
function generateDocumentation(portAssignments) {
  ...
  for (const assignment of portAssignments) {
    ...
    lines.push(
      `| ${app.displayName} | ${dir} | ${assignment.port} | ${assignment.source} | \`${script}\` |`
    );
  }
  ...
  if (bySource["reassigned"].length > 0) {
    ...
    for (const assignment of bySource["reassigned"]) {
      ...
      lines.push(
        `- **${assignment.app.displayName}**: Port ${assignment.port} (reassigned to resolve conflict)`
      );
...
async function main() {
  ...
  const outputFile =
    outputIndex >= 0 && args[outputIndex + 1] ? args[outputIndex + 1] : "ecosystem.config.js";
  ...
  for (const assignment of portAssignments) {
    const sourceLabel =
      assignment.source === "package.json"
        ? "from package.json"
        : assignment.source === "reassigned"
          ? "reassigned"
          : "auto-assigned";
