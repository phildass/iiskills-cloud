...
describe("content readiness — all scoped apps", () => {
      ...
      for (const file of files) {
        ...
        for (const field of REQUIRED_LESSON_FIELDS) {
          if (lesson[field] === undefined || lesson[field] === null) {
            errors.push(
              `${appSlug}/${path.relative(path.join(CONTENT_ROOT, appSlug), file)}: missing field "${field}"`
            );
          }
        }
        if (!Array.isArray(lesson.quiz) || lesson.quiz.length !== 5) {
          errors.push(
            `${appSlug}/${path.relative(path.join(CONTENT_ROOT, appSlug), file)}: expected 5 quiz questions`
          );
        }
    ...
    it("only module-1/lesson-1 has isFree: true; all others isFree: false", () => {
      ...
      for (const file of files) {
        ...
        if (!isM1L1 && lesson.isFree !== false) {
          errors.push(
            `${appSlug}/${path.relative(path.join(CONTENT_ROOT, appSlug), file)}: must have isFree: false`
          );
        }
      }
      expect(errors).toHaveLength(0);
    ...
    it("all lessons have isFree: true", () => {
      ...
      for (const file of files) {
        ...
        if (lesson.isFree !== true) {
          errors.push(
            `${appSlug}/${path.relative(path.join(CONTENT_ROOT, appSlug), file)}: must have isFree: true`
          );
        }
      }
      expect(errors).toHaveLength(0);
  ...
  describe.each(ALL_APPS)("%s — case studies and simulators", (appSlug) => {
    ...
    it("all case study and simulator files parse as valid JSON", () => {
      for (let i = 1; i <= 5; i++) {
        expect(() =>
          readJson(path.join(CONTENT_ROOT, appSlug, "case-studies", `case-${i}.json`))
        ).not.toThrow();
        expect(() =>
          readJson(path.join(CONTENT_ROOT, appSlug, "simulators", `sim-${i}.json`))
        ).not.toThrow();
      }
    });
  });
