/**
 * Tests for learn-management content integrity.
 *
 * Validates:
 ...
 *  - Only module-1/lesson-1 has isFree: true (paid-app sample rule)
...
const CONTENT_ROOT = path.resolve(__dirname, '../content/learn-management');
...
describe('learn-management content integrity', () => {
  describe('lesson files', () => {
    ...
    it('only module-1/lesson-1 has isFree: true (paid-app sample rule)', () => {
      const files = getAllLessonFiles();
      const errors = [];
      for (const file of files) {
        const lesson = readJson(file);
        const isModule1Lesson1 = lesson.moduleId === 1 && lesson.lessonId === 1;
        if (isModule1Lesson1 && lesson.isFree !== true) {
          errors.push(`${path.relative(CONTENT_ROOT, file)}: module-1/lesson-1 must have isFree: true`);
        }
        if (!isModule1Lesson1 && lesson.isFree !== false) {
          errors.push(`${path.relative(CONTENT_ROOT, file)}: non-sample lesson must have isFree: false`);
        }
      }
      expect(errors).toHaveLength(0);
    });
