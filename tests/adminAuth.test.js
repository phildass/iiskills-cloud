 ...
 * - validateAdminRequestAsync: delegates to validateAdminRequest (no Supabase Bearer token)
 */
...
// ---------------------------------------------------------------------------
// validateAdminRequest (sync — password/cookie only)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// validateAdminRequestAsync — delegates to validateAdminRequest (no Supabase)
// ---------------------------------------------------------------------------
...
describe("validateAdminRequestAsync", () => {
  ...
  test("accepts valid admin_session cookie", async () => {
    const req = makeReq({ headers: { cookie: adminCookieHeader() } });
  ...
  test("returns 403 when IP blocked", async () => {
    process.env.ADMIN_IP_ALLOWLIST = "10.0.0.1";
    const result = await validateAdminRequestAsync(makeReq());
    expect(result.valid).toBe(false);
    expect(result.status).toBe(403);
  ...
  test("does NOT accept Supabase Bearer token", async () => {
    const req = makeReq({
      headers: { cookie: "", authorization: "Bearer some-supabase-token" },
    });
    const result = await validateAdminRequestAsync(req);
    // Bearer tokens are no longer accepted — must use cookie or x-admin-secret
    expect(result.valid).toBe(false);
    expect(result.status).toBe(401);
  });
});
