...
This is a standalone Next.js application for the **Learn PR** module of iiskills.cloud. It is designed to be deployed as a separate service on the subdomain `learn-pr.iiskills.cloud`.
...
1. **Vercel** (Recommended)
   - Deploy directly from the `learn-pr` directory
   - Set domain to `learn-pr.iiskills.cloud`
   - Add environment variables in Vercel dashboard

2. **VPS with Nginx + PM2**
   - Build the app and run with PM2 on port 3008
   - Configure Nginx reverse proxy for `learn-pr.iiskills.cloud`
   - See main [DEPLOYMENT.md](../DEPLOYMENT.md) for details

3. **Docker**
   - Build Docker image with production build
   - Run container on port 3008
...
| `NEXT_PUBLIC_SITE_URL`          | App URL                | `https://learn-pr.iiskills.cloud` |
